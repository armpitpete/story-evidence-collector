#!/usr/bin/env python3
"""Verify the fixed commitments baseline with the exact later EDM extension."""
...
ORIGINAL_SCRIPT_PATH = (
    "scripts/"
    "test_jeremy_corbyn_current_parliament_commitments_and_public_positions_v1.py"
)
...
def load_original_namespace() -> dict[str, Any]:
    ...
    namespace: dict[str, Any] = {
        "__name__": "_fixed_commitments_public_positions_regression",
...
def main() -> int:
    if len(sys.argv) != 1:
        raise AssertionError("integration mode is not authorised in this regression repair")
    ...
    assert result == 0
    print(
        "PASS — fixed commitments/public-positions baseline preserved with "
        "the exact six-EDM fixture extension"
    )
    return 0
